...
# build-mac-local.sh
# Script to build macOS artifacts locally.
# Features:
...
for arg in "$@"; do
  case $arg in
    ...
    *)
      ...
      echo "Usage: ./build-mac-local.sh [--force] [--no-bump]"
      exit 1
      ;;
  esac
done
...
if [ ! -d "node_modules" ]; then
  ...
  echo "Please run 'npm install' before building."
  exit 1
fi
...
if [ -f .env.local ]; then
  ...
  # Clean up the .env.local lines to be exportable (simple version)
...
if [ "$FORCE_BUILD" = false ] && [ -f "$HISTORY_FILE" ]; then
  LAST_HASH=$(run_node "
    try {
      ...
      if (history.mac && history.mac.hash) console.log(history.mac.hash);
    } catch (e) {}
  ")
  
  if [ "$CURRENT_HASH" = "$LAST_HASH" ]; then
    echo "This commit ($CURRENT_HASH) was already built for macOS."
...
# 4. Run Build
echo "Preparing dist/mac directory..."
mkdir -p dist/mac
# Clear existing contents
rm -rf dist/mac/*

echo "Starting macOS build..."
npx electron-builder --mac --config.directories.output=dist/mac
