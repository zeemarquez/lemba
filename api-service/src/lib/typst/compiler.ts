/**
 * Server-side port of the Typst compiler used by the Modern Markdown Editor.
 *
 * This module is intentionally kept structurally identical to the original
 * `lib/typst/client-compiler.ts` so that the generated Typst source is
 * 100% compatible with what the Electron / web app produces. Differences:
 *   - Removed the `'use client'` directive (this runs in Node.js).
 *   - WASM and font assets are fetched from a CDN at startup (Node 18+ has
 *     a global `fetch`).
 *   - `compileTypstToPdf` is exposed for direct use by the API service.
 */
