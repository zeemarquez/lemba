        ...
        switch (token.type) {
            ...
            case 'html': {
                ...
                if (/^<br\s*\/?>\s*$/i.test(htmlVal)) {
                    ...
                    // Handle inline <img> tags — convert to #image() to avoid raw "<" in Typst markup
                    const inlineImgMatch = htmlVal.match(/<img\s+[^>]*src=["']([^"']+)["'][^>]*>/i);
                    if (inlineImgMatch) {
                        const inlineSrc = inlineImgMatch[1];
                        const inlineWidthMatch = htmlVal.match(/width[:=]\s*["']?(\d+(?:px|%)?)["']?/i);
                        const inlineW = inlineWidthMatch ? fixTypstUnit(inlineWidthMatch[1]) : '';
                        output += `#image("${escapeTypstString(inlineSrc)}"${inlineW ? `, width: ${inlineW}` : ''})`;
                    } else {
                        // Strip HTML tags from other inline HTML to avoid "<" causing "unclosed label"
                        const stripped = htmlVal.replace(/<[^>]*>/g, '');
                        if (stripped) output += processTextWithEmojis(stripped);
                    }
                }
                break;
            }
