...
function serializeTable(element: TElement, context: SerializeContext): string {
    ...
    rows.forEach(row => {
        (row.children as TElement[]).forEach(cell => {
            ...
            const cellAlignAttr = (cell as any).align as string | undefined;
            const alignParts: string[] = [];
            if (verticalAlign === 'middle') alignParts.push('horizon');
            else if (verticalAlign === 'bottom') alignParts.push('bottom');
            if (cellAlignAttr === 'center') alignParts.push('center');
            else if (cellAlignAttr === 'right') alignParts.push('right');
            else if (cellAlignAttr === 'left') alignParts.push('left');
            else {
                const hz = isHeader ? headerTextAlign : cellTextAlign;
                if (hz === 'center') alignParts.push('center');
                else if (hz === 'right') alignParts.push('right');
                else alignParts.push('left');
            }
            cellArgs.push(`align: ${alignParts.join(' + ')}`);

            const cellBg = (cell as any).background as string | undefined;
            const templateBg = isHeader ? headerBgColor : cellBgColor;
            const fillColor = cellBg || templateBg;
            if (fillColor) {
                cellArgs.push(`fill: rgb("${fillColor}")`);
            }
